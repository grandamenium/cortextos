...
// Renamed from `cortextos doctor` to `cortextos doctor-deps` on 2026-05-18 when
// the new `doctor` command became the auto-remediation counterpart to `status`.
// This file is the original prerequisites probe (PM2 / Claude CLI / node-pty /
// tunnel / GEMINI_API_KEY / etc.) — it answers "is my install wired correctly?"
// while the new doctor answers "the install is wired; what's wrong RIGHT NOW
// and can you fix it?".
export const doctorDepsCommand = new Command('doctor-deps')
  .option('--instance <id>', 'Instance ID', 'default')
  .description('Diagnose connectivity / dependency prerequisites (PM2, Claude CLI, node-pty, tunnel, GEMINI key)')
  .action(async (options: { instance: string }) => {
    console.log('\ncortextOS Doctor (deps)\n');
