...
export const ecosystemCommand = new Command('ecosystem')
  ...
  .action(async (options: { instance: string; org?: string; output: string }) => {
    ...
    // Windows PATH override: PM2 spawns the daemon with a minimal environment
    // that does not inherit the user's full PATH. Without this, the daemon
    // cannot find claude.cmd (~/.local/bin or AppData/Roaming/npm) and agent
    // hooks that shell to `cortextos` return 127. Prepend the three relevant
    // directories so spawned agents and hooks resolve correctly.
    // On Mac/Linux, PM2 inherits the user's PATH, so no override is needed.
    const pathOverrideLine = IS_WINDOWS
      ? `\n        PATH: [path.join(os.homedir(), 'bin'), path.join(os.homedir(), '.local', 'bin'), path.join(os.homedir(), 'AppData', 'Roaming', 'npm'), process.env.PATH || ''].join(path.delimiter),`
      : '';
    const requireHeader = IS_WINDOWS
  ...
  apps: [
    {
      ...
      env: {
        ...
        CTX_ORG: process.env.CTX_ORG || ${JSON.stringify(detectedOrg)},${pathOverrideLine}
      },
      max_restarts: 50,
      restart_delay: 5000,
      autorestart: true,
