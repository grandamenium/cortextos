  ...
  .action(async (name: string, options: { template: string; org?: string; instance: string; runtime: string }) => {
    ...
    if (!existsSync(envPath)) {
      writeFileSync(envPath, [
        ...
        '# ALLOWED_USER: Your numeric Telegram user ID. The same curl above prints',
        '#   .result[-1].message.from.id — paste that number here. Must be numeric,',
        '#   not a username/handle. If missing or non-numeric, the daemon refuses to',
        '#   start the inbound Telegram poller (security: prevents arbitrary users',
        '#   from controlling the agent via @bot handle).',
        '#',
        'BOT_TOKEN=',
        'CHAT_ID=',
        'ALLOWED_USER=',
