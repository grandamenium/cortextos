...
import { findOrgTemplateDir, copyOrgTemplateFiles } from './templates.js';
import type { OrgContext } from '../types/index.js';

export interface InitOrgOptions {
  orgName: string;
  /** Instance ID (state root under ~/.cortextos/<instance>). Default 'default'. */
  instance?: string;
  /** Repo/project root where orgs/<org>/ lives. Default process.cwd(). */
  projectRoot?: string;
  /**
   * Regenerate SYSTEM.md for already-existing agents from the now-current
   * context.json. Default true — preserves the `cortextos init` upgrade
   * behavior. The pack installer disables this and regenerates once after its
   * full roster + context.orchestrator are in place.
   */
  regenerateExistingSystemMd?: boolean;
}

/**
 * Scaffold a cortextOS organization on disk. Extracted verbatim from the `init`
 * command's action (F3) so the `add-org --pack` installer can reuse it without
 * going through Commander. Behavior-preserving: never-overwrite for org files,
 * context.json missing-field backfill, and (toggleable) SYSTEM.md regen for
 * existing agents. Throws on unexpected I/O errors rather than process.exit;
 * the caller is responsible for validating orgName (validateOrgName) first.
 */
export function initOrg(opts: InitOrgOptions): void {
  const orgName = opts.orgName;
  const instanceId = opts.instance ?? 'default';
  const projectRoot = opts.projectRoot ?? process.cwd();
  const regenerateExistingSystemMd = opts.regenerateExistingSystemMd ?? true;
  ...
  if (regenerateExistingSystemMd && existsSync(agentsDir)) {
