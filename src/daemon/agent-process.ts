...
export class AgentProcess {
  ...
  private buildStartupPrompt(): string {
    ...
    return `You are starting a new session. Current UTC time: ${nowUtc}. Read AGENTS.md and all bootstrap files listed there. Then restore your crons from config.json: CRITICAL DEDUP: Always call CronList BEFORE creating any cron. For each config.json entry, search the CronList output for its prompt text — if the prompt already appears, SKIP that cron entirely. For entries NOT already listed: for each entry with type "recurring" (or no type field), QUEUE the CronCreate call (do NOT call immediately). For entries with type "once": compare fire_at against the current UTC time — if fire_at is in the future QUEUE the CronCreate call, if in the past delete from config.json. After queueing all CronCreate calls: WAIT for the dispatcher-ready marker by polling for existence of \`$CTX_ROOT/state/.dispatcher-ready\` (check every 100ms, timeout after 30s). Once the marker exists OR timeout, FLUSH all queued CronCreate calls by executing them. This ensures the daemon dispatcher is ready before crons register. Note: do NOT use /loop — /loop will prompt about cloud scheduling which blocks boot.${reminderBlock}${deliverablesBlock}${handoffBlock}${handoffUxOverride}${onlineMessage}${onboardingAppend}`;
  }
