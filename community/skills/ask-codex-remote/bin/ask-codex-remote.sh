...
#
# Usage: ask-codex-remote.sh "<prompt>" [--resume] [--timeout <seconds>]
# Env:
...
#   CODEX_TIMEOUT (default 30, overridden by --timeout flag)
#
...
if [ -n "$LOCAL_CODEX" ] && [ -f "$HOME/.codex/auth.json" ]; then
  BACKEND="local"
  T0=$(date +%s)
  timeout "$TIMEOUT" env -u OPENAI_API_KEY "$LOCAL_CODEX" exec --skip-git-repo-check $RESUME_FLAG "$PROMPT" >"$OUT_FILE" 2>"$WORK/codex.err" &
  CODEX_PID=$!
  wait $CODEX_PID
  RC=$?
  ELAPSED=$(($(date +%s) - T0))
  ...
  timeout "$TIMEOUT" ssh -F "$SSH_CONFIG" -o ConnectTimeout=10 -o LogLevel=ERROR "$REMOTE_HOST" \
    "export PATH=/opt/homebrew/bin:\$HOME/.local/bin:\$PATH; cd /tmp && timeout $TIMEOUT codex exec --skip-git-repo-check $RESUME_FLAG \"\$(echo $PROMPT_B64 | base64 -d)\"" \
