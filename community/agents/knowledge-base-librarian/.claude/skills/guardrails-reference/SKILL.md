...
| Creating a recurring cron | "A session-local scheduler is enough" | Session-local scheduling does not survive restarts. Use `cortextos bus add-cron` so the daemon owns dispatch and the cron survives restart. |
