...
| About to spawn a subagent or run a heavy/looping job | "I'll just have it read every item / process the whole thing" | BOUND IT. Hard-cap tool-calls per subagent (~50); NO per-item/per-thread reads in a loop (batch or sample instead). Unbounded jobs trip the account usage cap (a 1,319-call run froze the whole fleet 2026-06-24). Heavy jobs must be chunked AND usage-gated (run only when 5h usage < 0.50, else defer). See the usage playbook in `knowledge.md`. |
