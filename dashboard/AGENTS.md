...
- **Cost view** — `dashboard/src/lib/cost-parser.ts` walks both `~/.claude/projects/*.jsonl` (claude transcripts) and `<ctxRoot>/logs/<agent>/codex-tokens.jsonl` (codex cumulative-token flat-schema log). It converts each session's cumulative counters to per-turn deltas, then merges both sources into a single `CostEntry[]` with a persisted `(source_file, timestamp, model, agent)` identity. `gpt-5-codex` pricing is in `MODEL_PRICING` ($1.25/M input, $10/M output, $0.125/M cache_read, $0/M cache_write) with substring matching on `codex` or `gpt-5` in `resolvePricingKey()`.
