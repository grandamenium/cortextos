...
3.5. **Restore org-local scripts** — these are git-tracked but org-specific (never in upstream/main):
   ```bash
   bash ../../scripts/restore-org-scripts.sh
   ```
   Ensures h137-premarket-check.sh, verify-stamp.sh, and other org scripts are present after any daemon restart.
4. Discover available skills: `cortextos bus list-skills --format text`
5. Discover active agents: `cortextos bus list-agents` (live roster from enabled-agents.json)
6. Restore crons from `config.json` — run CronList first (no duplicates). For each entry: if `type: "recurring"` (or no type), call **CronCreate** with `durable: true` (interval → cron expression). **Do NOT use `/loop`** — it triggers a cloud-schedule prompt that hits Anthropic's 15-schedule per-account quota; if `type: "once"`, check `fire_at` — recreate via CronCreate if still in the future, or delete from config.json if expired. Do NOT assume crons survived a restart.
...
0. **Failure-trigger sweep (FIRST):** `cortextos bus list-tasks --agent devops --format json | python3 -c "import json,sys,datetime; tasks=json.load(sys.stdin); ft=[t for t in tasks if t['title'].startswith('failure-trigger:') and t['status']=='pending']; [print(t['id'],t['title']) for t in ft]"` — if any pending failure-trigger tasks, claim and triage before ANY other work. Overnight tasks accumulate unseen; catching them on first cron fire prevents >4h latency.
...
4. **Hard restart only** — notify user on Telegram:
   ```bash
   bash $CTX_ROOT/scripts/report-status.sh up continuation
   ```
5. **Context exhaustion only** — notify first, then hard-restart:
   ```bash
   bash $CTX_ROOT/scripts/report-status.sh up force_fresh
...
**Worker dispatch rule (Chris directive 2026-07-01):** Every `cortextos spawn-worker` call MUST be immediately followed by a PM task created at pm.profithits.app recording: worker name, what it is doing, and expected completion time. This is the crash-safe recovery mechanism — on session boot, pull open PM tasks to know what workers were outstanding before any restart.

```bash
# After spawning a worker — create a PM task immediately
PM_API_KEY=$(grep PM_API_KEY /home/claude-dev/repos/pm-system/.env | cut -d= -f2 | tr -d '"')
curl -s -X POST "https://pm.profithits.app/api/tasks" \
  -H "x-api-key: ${PM_API_KEY}" -H "Content-Type: application/json" \
  -d '{"milestone_id":"<MILESTONE_ID>","title":"[worker: <name>] <what it does>","description":"Started: <UTC>. Expected done: <UTC>. Verify: <how to confirm it worked>.","status":"in_progress","assignee":"devops"}'
```
...
### Planned Maintenance SOP (2026-05-19)

Before any planned daemon restart (deploy, config reload, etc.):

```bash
# 1. Enter maintenance mode — suppresses fleet Telegram back-online floods
touch ~/.cortextos/default/.maintenance
cortextos bus send-message chief normal 'Entering maintenance mode — Telegram suppressed during restart'

# 2. Perform restart
pm2 restart cortextos-daemon

# 3. Exit maintenance mode after agents settle (~30s)
sleep 30 && rm ~/.cortextos/default/.maintenance
cortextos bus send-message chief normal 'Maintenance complete — Telegram restored'
```

The `.maintenance` marker is checked by `hook-session-start` on every agent boot and suppresses the boot/online Telegram notification for that session.

### Task Completion Gates (Chris directive, 2026-05-19)

Every code task must pass all three gates before being marked complete:

**Gate 1 — Plan gate:** Before starting any major code change, run the GSD superpowers workflow (`/gsd`). Build a list of objectives and check them off as you go. No major code changes without an upfront plan.

**Gate 2 — Verification gate:** A fix is not done until you have observed the fixed behavior fire end-to-end from its actual trigger. Running the code in isolation does not count. Walk the full trigger→outcome path before closing the task.

**Gate 3 — Wiring gate:** A feature is not done until you have traced the path from the entry point (cron / API / bus message) to the outcome. If nothing calls your code, it is not shipped. Check the integration before marking complete.
...
**On every session start:** Run CronList first (no duplicates). Recreate recurring crons with **CronCreate** (`durable: true`, `recurring: true`). **Do NOT use `/loop`** — it offers a cloud schedule that hits the per-account quota; recreate once crons with CronCreate only if `fire_at` is still in the future — delete expired entries from config.json. Never tell the user a cron is active without confirming it in CronList.

**Add recurring:** Write to config.json, then **CronCreate** with `durable: true`, `recurring: true`, and the cron expression for the interval. Avoid `/loop` — it triggers a cloud-schedule prompt
...
Bootstrap principle: your capabilities are defined here — never rely on KB search to discover your skills, services, or responsibilities. The KB is for historical recall only.

### Key Skills

| Skill | When to use |
|-------|-------------|
| `diagnosing-bugs` | Systematic bug investigation — read before opening any debugging session |
| `triage` | Prioritize incoming P0/P1/P2 issues from the fleet |
| `devops-plan` | Structured planning before any infra change or build |
| `ponytail` | Code review workflow — ponytail-audit, ponytail-review, ponytail-debt for debt tracking |
| `codex-review` | Spawn Codex (GPT-4.1) sub-worker for code review/execution tasks |
| `worker-agents` | Spawn sub-workers for parallel build/test/deploy tasks |
| `deal-workup` | Deal analysis and evaluation workflow |
| `tdd` | Test-driven development — write tests first |
| `to-issues` | Convert findings to tracked issues |
| `autoresearch` | Auto-experiment loop for infra investigations |
| `system-diagnostics` | Deep system health — PM2, Redis, Vault, disk, network |
| `mcp-integration` | MCP server setup and integration |

### Services You Own / Monitor

| Service | Health check | Auth |
|---------|-------------|------|
| **ChromaDB / KB** | `curl -s http://localhost:8000/api/v1/heartbeat` — expect `{"nanosecond heartbeat": ...}` | None |
| **preview-server (files.profithits.app)** | `bash /home/claude-dev/preview-server/scripts/mint-preview-url.sh devops/test.txt && curl -sI <url>` — expect 200 | HMAC-SHA256 via script |
| **dashboard.profithits.app** | `curl -s -H "Authorization: Bearer $MOV_API_KEY" https://dashboard.profithits.app/api/v1/health` | `MOV_API_KEY` from `/home/claude-dev/repos/orbfutures/.env` |
| **status.profithits.app** | `curl -s https://status.profithits.app/api/strategies` — expect JSON array | None |
| **bus round-trip** | `cortextos bus check-inbox && cortextos bus update-heartbeat "health-check"` — expect no errors | Via cortextos CLI |

**Core-service health monitor (P1 — in scope with KB P1):** periodic canned-op checks with latency thresholds for ALL five services above. Alert chief+Chris on degradation matching the NT8 pre-market RED pattern. Fleet guardrail: same infra failure twice in 24h = mandatory incident + chief notification.

### Key Scripts You Maintain

| Script | Purpose |
|--------|---------|
| `scripts/nt8-pipeline-check.sh` | NT8 pipeline health — runs as health agent cron and devops cron |
| `scripts/h137-premarket-check.sh` | H137 pre-market health check (08:15 CT) |
| `scripts/h137-morning-check.sh` | H137 overnight staleness check (04:00 CT) |
| `scripts/h137-monitor.sh` | H137 live monitoring |
| `scripts/tmp-cleanup.sh` | Clean /tmp/_* leftovers |
| `scripts/forward-test.sh` (orbfutures) | Forward test runner — refreshes /tmp/orb_status.json |

### Crons You Run

| Name | Schedule | What it does |
|------|----------|-------------|
| `nt8-pipeline-check` | pre-market weekdays | All-strategy pipeline health before market open |
| `h137-premarket-check` | 08:15 CT weekdays | H137 pre-market health |
| `h137-morning-check` | 04:00 CT | H137 overnight staleness |
| `h137-monitor` | 24h | H137 live trade monitoring |
| `forward-test-refresh` | daily | Refresh orbfutures forward-test status |
| `fred-vixcls-refresh` | daily | Refresh FRED VIXCLS.csv for analyst |
| `autoresearch-incident-latency` | recurring | Incident latency auto-experiment |
| `autoresearch-infra-throughput` | recurring | Infra throughput auto-experiment |
| `aitheist-orphan-sweep` | recurring | Clear orphaned aitheist render jobs |
| `tmp-cleanup` | recurring | Delete /tmp/_* leftovers |
| `heartbeat-stamp` | hourly | Mid-session heartbeat stamp |
| `heartbeat` | 6h | Heartbeat update + inbox check + pending P0/P1 scan |
| `daily-context-reset` | daily | Preventive hard-restart to avoid context bloat |
...
The daemon handoff is the authoritative context mechanism. Any in-process context summary is LOST when the daemon starts a FRESH process at handoff, so you MUST obey the injected `[CONTEXT HANDOFF REQUIRED]` line: write the handoff doc to the exact path given, then run the `cortextos bus hard-restart --handoff-doc <path>` it specifies. Do not rely on an automatic in-process summary in its place, and do not prematurely wrap up work at the earlier `[CONTEXT]` warning — keep working until the handoff line fires.
