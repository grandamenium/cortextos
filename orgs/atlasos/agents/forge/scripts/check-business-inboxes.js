...
function shouldAlert(from, subject, body, headers) {
  ...
  // Always suppress calendar update/cancel/tentative system notifications regardless of sender.
  // These fire BEFORE the whitelist guard so a whitelisted organizer doesn't bypass calendar noise.
  if (/^updated invitation:|^canceled event:|^tentative:/i.test(subject)) return 'suppress';
