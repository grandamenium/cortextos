...
describe('getOrgs case-insensitive dedupe', () => {
  it('same org in both dirs with drifted casing: returns ONE entry using framework casing', () => {
    resetOrgs();
    // This is the exact scenario observed in production: canonical ElementOneSound
    // in framework root, stale lowercase elementonesound in state dir from a
    // historical kb-* invocation with a lowercase --org argument.
    mkorg('framework', 'ElementOneSound');
    mkorg('state', 'elementonesound');
    mkorg('state', 'ElementOneSound');

    const orgs = getOrgs();
    expect(orgs).toHaveLength(1);
    expect(orgs[0]).toBe('ElementOneSound');
  });
  ...
  it('multiple orgs: framework wins per-org, state-only orgs included', () => {
    resetOrgs();
    // agentnet exists in both with matching casing; ElementOneSound has drift;
    // stateonly exists only in state.
    mkorg('framework', 'agentnet');
    mkorg('state', 'agentnet');
    mkorg('framework', 'ElementOneSound');
    mkorg('state', 'elementonesound');
    mkorg('state', 'stateonly');

    const orgs = getOrgs().sort();
    expect(orgs).toEqual(['ElementOneSound', 'agentnet', 'stateonly']);
  });
});
